...
interface StandaloneFrameworkNoteGeneratorProps {
  onNoteCreated?: () => void;
}

export const StandaloneFrameworkNoteGenerator: React.FC<StandaloneFrameworkNoteGeneratorProps> = ({ 
  onNoteCreated 
}) => {
  ...
  const [noteTitle, setNoteTitle] = useState('Nouvelle note de cadrage');
  const [prompt, setPrompt] = useState(`Génère une note de cadrage.`);
  ...
  const generateWithAI = async (sectionToGenerate: string, customPrompt?: string) => {
    ...
    try {
      ...
      const sectionPrompt = customPrompt || `Génère la section "${sectionLabel}" pour une note de cadrage intitulée "${noteTitle}".`;
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      setGeneratedContent({});
      setNoteTitle('Nouvelle note de cadrage');
      setActiveTab('editor');
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        <div>
          <Label htmlFor="note-title">Titre de la note de cadrage</Label>
          <Input 
            id="note-title"
            value={noteTitle}
            onChange={(e) => setNoteTitle(e.target.value)}
            placeholder="Entrez le titre de la note de cadrage"
            className="w-full"
          />
        </div>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="border rounded-lg p-4 bg-muted/50">
                <h3 className="font-medium mb-2">1. Donnez un titre à votre note</h3>
                <p className="text-sm text-muted-foreground">
                  Le titre doit être concis et descriptif pour faciliter l'identification de la note.
                </p>
              </div>
              
              <div className="border rounded-lg p-4 bg-muted/50">
                <h3 className="font-medium mb-2">2. Générer du contenu</h3>
                <p className="text-sm text-muted-foreground mb-2">
                  Vous pouvez générer le contenu de chaque section individuellement ou toutes les sections en une seule fois.
                </p>
                <p className="text-sm text-muted-foreground">
                  Le contenu généré peut être modifié selon vos besoins.
                </p>
              </div>
              
              <div className="border rounded-lg p-4 bg-muted/50">
                <h3 className="font-medium mb-2">3. Personnaliser chaque section</h3>
                <p className="text-sm text-muted-foreground mb-2">
              ...
              <div className="border rounded-lg p-4 bg-muted/50">
                <h3 className="font-medium mb-2">4. Enregistrer la note</h3>
                <p className="text-sm text-muted-foreground">
                  Une fois satisfait du contenu, cliquez sur "Enregistrer la note" pour sauvegarder la note de cadrage.
                </p>
              </div>
            </CardContent>
          </Card>
