...
interface StandaloneFrameworkNoteEditProps {
  note: any;
  isOpen: boolean;
  onClose: () => void;
}

export const StandaloneFrameworkNoteEdit: React.FC<StandaloneFrameworkNoteEditProps> = ({
  note,
  isOpen,
  onClose
}) => {
  ...
  useEffect(() => {
    if (note) {
      ...
      setNoteTitle(note.title || `Note de cadrage v${note.version}`);
    }
  }, [note]);
    ...
    try {
      ...
      const response = await fetch(`${process.env.SUPABASE_URL}/functions/v1/ai-assistant`, {
        ...
        body: JSON.stringify({
          messages: [{ 
            role: 'user', 
            content: `Génère la section "${sectionLabel}" pour une note de cadrage intitulée "${noteTitle}".` 
          }],
