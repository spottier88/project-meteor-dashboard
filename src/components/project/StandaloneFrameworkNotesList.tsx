...
import { Pencil, Eye, Trash, Link, FolderPlus } from 'lucide-react';
...
interface StandaloneFrameworkNotesListProps {
  onViewNote: (note: any) => void;
  onEditNote: (note: any) => void;
  onLinkNote: (note: any) => void;
  canEdit: boolean;
}

export const StandaloneFrameworkNotesList: React.FC<StandaloneFrameworkNotesListProps> = ({ 
  onViewNote,
  onEditNote,
  onLinkNote,
  canEdit 
  ...
  const { data: notes, isLoading, refetch } = useQuery({
    queryKey: ['standaloneFrameworkNotes'],
    queryFn: async () => {
  ...
  if (!notes || notes.length === 0) {
    return (
      <div className="text-center p-8 bg-muted rounded-lg">
        <p className="text-muted-foreground">Aucune note de cadrage indépendante n'a été créée</p>
      </div>
    );
  }
        ...
        <Card key={note.id}>
          <CardHeader>
            <CardTitle className="flex justify-between">
              <span>{note.title || `Note v${note.version}`}</span>
          ...
          <CardFooter className="flex justify-between">
            ...
            {canEdit && (
              <>
                ...
                <Button variant="outline" size="sm" onClick={() => onLinkNote(note)}>
                  <Link className="h-4 w-4 mr-1" /> Lier
                </Button>
                <AlertDialog>
