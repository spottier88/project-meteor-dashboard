-- Mise à jour de la fonction get_detailed_projects pour inclure le champ difficulties
...
BEGIN
    ...
    LOOP
        -- Récupérer la dernière revue (avec le champ difficulties ajouté)
        SELECT 
            jsonb_build_object(
                'weather', lr.weather,
        ...
        IF project_rec.project_manager IS NOT NULL THEN
            SELECT CONCAT(first_name, ' ', last_name) INTO project_manager_name
            FROM profiles
            WHERE email = project_rec.project_manager;
        END IF;
        
        -- Récupérer le nom de l'entité destinataire (for_entity)
        IF project_rec.for_entity_id IS NOT NULL AND project_rec.for_entity_type IS NOT NULL THEN
            CASE project_rec.for_entity_type
                WHEN 'pole' THEN
                    SELECT name INTO for_entity_name FROM poles WHERE id = project_rec.for_entity_id;
                WHEN 'direction' THEN
                    SELECT name INTO for_entity_name FROM directions WHERE id = project_rec.for_entity_id;
                WHEN 'service' THEN
                    SELECT name INTO for_entity_name FROM services WHERE id = project_rec.for_entity_id;
            END CASE;
        END IF;
        
        -- Construire l'objet JSON pour ce projet
        project_json = jsonb_build_object(
            'project', jsonb_build_object(
                ...
                'completion', COALESCE(review_data->>'completion', '0')::INTEGER,
                'weather', review_data->>'weather'
            ),
            'lastReview', CASE WHEN review_data IS NOT NULL 
                THEN (review_data || jsonb_build_object('actions', review_actions)) - 'review_id'
                ELSE NULL
            END,
        ...
        -- Ajouter l'objet projet au résultat
        result = result || jsonb_build_array(project_json);
    END LOOP;
